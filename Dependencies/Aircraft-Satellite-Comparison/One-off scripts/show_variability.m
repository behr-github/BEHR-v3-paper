function [  ] = show_variability( airno2_iall, lat_iall, lon_iall, dates_iall, db_iall, campaign_name, ptskip, use_cl_instead, tosave )
...
clean_campaign_name = upper(regexprep(campaign_name,'\W','_'));
merge_name = [clean_campaign_name,'_P3_1sec_%s_%s_%s.mat'];

% Load the month-averaged NO2 columns for this campaign. (Creates variables
% LON_GRID, LAT_GRID, and NO2_GRID.
F = dir(sprintf('/Users/Josh/Documents/MATLAB/NO2 Profiles/Workspaces/%s*AvgBEHRNO2.mat',clean_campaign_name));
if numel(F) == 1
    load(fullfile('/Users/Josh/Documents/MATLAB/NO2 Profiles/Workspaces',F(1).name));
elseif numel(F)==0
    error(E.filenotfound('avg. campaign NO2 map'));
else
    error(E.toomanyfiles('avg. campaign NO2 map'));
end
...
for a=1:numel(dates_to_load)
    ...
    for d=1:nd
        ...
        hold on
        [this_lon,this_lat] = PlotAircraftData(no2,lon,lat,profnums,profnum_dd(xx),'[NO_2] (pptv)',ptskip);
        
        subplot(2,2,3)
        
        set(gca,'fontsize',16);
        title(sprintf('%s: Profiles %s\n(LST: %s)\nAltitude',dates_to_load{a},mat2str(profnum_dd(xx)),lsts));
        hold on
        PlotAircraftData(alt,lon,lat,profnums,profnum_dd(xx),'Alt (km)',ptskip);
        
        curr_xlim = get(gca,'xlim');
        curr_ylim = get(gca,'ylim');
        
        DrawPcolorMap(this_loncorn, this_latcorn, this_lon, this_lat, NO2_GRID, LON_GRID, LAT_GRID, curr_xlim, curr_ylim, collim);
        set(gca,'fontsize',16);
        title(sprintf('%s: Profiles %s\n(LST: %s)\nCampaign avg. NO2',dates_to_load{a},mat2str(profnum_dd(xx)),lsts));
...
function [sublon, sublat] = PlotAircraftData(data_in,lon,lat,raw_profnums,matched_profnums,cblabel,ptskip)
    ...
    sublon = lon(xx);
    sublat = lat(xx);
    scatter(lon(xx),lat(xx),12,data_in(xx));
    cb=colorbar;
...
function DrawSatPix(no2,loncorn,latcorn,clim)
    ...
    for a=1:numel(no2)
        % Find the closest cvec value to each no2 value, this will be the color
        % of each pixel
        [~,i] = min(abs(no2(a)-cvec));
        col = cmap(i,:);
        
        subplot(2,2,1);
        phandle = patch(loncorn(:,a),latcorn(:,a),'w','edgecolor','k','facealpha',0.9,'linewidth',2);
        set(phandle,'facecolor',col)
        
        subplot(2,2,2);
        patch(loncorn(:,a),latcorn(:,a),'w','edgecolor',col,'linewidth',2);
...
function DrawPcolorMap(loncorn, latcorn, airlon, airlat, NO2_GRID, LON_GRID, LAT_GRID, lonlim, latlim, collim)
    subplot(2,2,4);
    xx = LON_GRID(1,:) >= min(lonlim) & LON_GRID(1,:) <= max(lonlim);
    yy = LAT_GRID(:,1) >= min(latlim) & LAT_GRID(:,1) <= max(latlim);
    pcolor(LON_GRID(yy,xx), LAT_GRID(yy,xx), NO2_GRID(yy,xx));
    shading flat
    caxis(collim)
    colorbar;
    ...
    x = [];
    y = [];
    for a=1:size(loncorn,2)
        x = [x; loncorn(:,a); loncorn(1,a); nan];
        y = [y; latcorn(:,a); latcorn(1,a); nan];
    end
    
    line(x,y,'color','w','linewidth',2);
    
    line(airlon, airlat, 'color',[0.8 0.8 0.8]);
